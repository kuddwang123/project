...
void TestLoadSo::callback1(const hj_bf::HJTimerEvent &)
{
  ...
  std::cout << std::fixed << std::setprecision(6) << ros::Time().fromSec(test)<< std::endl; 
  ...
  std::cerr<<"minos here now"<<std::endl;
...
//  std::cerr << "minos TestLoadSo:" << temp_timer2.use_count() << std::endl;
  ...
  //  hj_bf::createVariable("minos_test1", num);
   timer1 = hj_bf::HJCreateTimer("timer1", 10 * 1000 * 1000, &TestLoadSo::callback1, this);
...
//  timer3.stop();
//  auto state = std::thread(&loopPrint2);  // 开线程
  // state.detach();
